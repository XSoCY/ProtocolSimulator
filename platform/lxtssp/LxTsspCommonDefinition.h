...
typedef struct ModuleInfoStruct
{
    ...
    ModuleInfoStruct()
    {
        strcpy(szCompany, QStringLiteral("中国航天科技集团九院七〇四所").toUtf8().data());
        memset(szModuleName, 0, sizeof(char) * 32);
        memset(szConfigID, 0, sizeof(char) * 32);
        memset(szVersion, 0, sizeof(char) * 16);
        memset(szCreator, 0, sizeof(char) * 24);
        memset(szComment, 0, sizeof(char) * 64);
    }
...
typedef struct ObjectInfo
{
    ...
    ObjectInfo()
    {
        ...
        usObjectID = 0;
        ucSN = 0;
        memset(szAlias, 0, sizeof(char) * 64);
        memset(szComment, 0, sizeof(char) * 64);
        bLocalObj = false;
        bMainUse = false;
    }
...
typedef struct UserInformation
{
    ...
    UserInformation()
    {
        ...
        memset(szPassword, 0, sizeof(char) * 16);
        iRole = -1;
        bLocalUser = true;
        memset(szContent, 0, sizeof(char) * 32);
        memset(szIP, 0, sizeof(char) * 32);
        uiDisconnectCount = 0;
        memset(uiDeviceID, 0, sizeof(unsigned int) * 32);
        memset(uiDevMgrRole, 0, sizeof(unsigned int) * 32);
    }
...
public:
     ...
     */
    virtual int ReadData(quint64 ui64DataID, uchar* aData, uint& uiDataLen, uint& uiReloadCount) = 0;
...
public:
     ...
     */
    virtual int Login(char* szUserName, char* szPassword, char* szIP, uint uiLoginType, TLxTsspUserInformation& newInfo) = 0;
...
public:
     ...
     * @return 1：成功；0：宏不存在；-1：失败。
     */
    virtual int GetMacroInformation(uint uiMacroID, TLxTsspParamMacroInformation& macroInfo) const = 0;
...
public:
     ...
     * @return 1：成功；0：未找到指定的配置宏；-1：失败。
     */
    virtual int GetConfigMacroInformation(ulong ulMacroID, TLxTsspDevConfigMacroInformation& configMacroInfo) = 0;
...
public:
     ...
     * @return 1：成功；0：宏不存在；-1：失败。
     */
    virtual int GetMacroInformation(uint macroID, TLxTsspTaskMacroInformation& macroInfo) = 0;
...
//通道数据转发处理函数定义
typedef void (*ChannelDataTransferHandler)(
    unsigned char aData[], int& iDataLen, unsigned int& uiPackageCount, unsigned char aRemoteIP[], int& iRemotePort, int& iDataHeader);
...
public:
     ...
     */
    virtual int ReadReceivedData(uchar*& aData, int& iDataLen, uint& uiPackageCount, uchar*& aIP, int& iPort, int& iDataHeader) = 0;
     ...
     */
    virtual int RegisterReceiveReceivedData(ChannelDataTransferHandler processFunction, char* szHostName) = 0;
     ...
     * @return 1：成功；0：已注册过；-1：失败。
     */
    virtual int UnRegisterReceiveReceivedData(ChannelDataTransferHandler processFunction, char* szHostName) = 0;
     ...
     * @return 大于等于0：写入数据的实际长度。-1：通道未创建。
     */
    virtual int SendDataToIPEndPoint(uchar aData[], int iDataLen, uchar* aRemoteIp, int iRemotePort) = 0;
     ...
     * @return 大于等于0：数据包长度；-1：读取失败。
     */
    virtual int ReadSendedData(uchar*& aData, int& iDataLen, uint& uiPackageCnt, uchar*& aRemoteIp, int& iRemotePort, int& iDataHeader) = 0;
     ...
     * @return 1：成功；0：已注册过；-1：失败。
     */
    virtual int RegisterReceiveSendedData(ChannelDataTransferHandler processFunction, char* szHostName) = 0;
     ...
     */
    virtual int UnRegisterReceiveSendedData(ChannelDataTransferHandler processFunction, char* szHostName) = 0;
...
public:
     ...
     * @return 大于等于0：实际发送的数据长度；-1：发送失败。
     */
    virtual int SendDataToChannelAddress(QString strChannelID, uchar aData[], int iDataLen, uchar* aRemoteIP, int iRemotePort) = 0;
    ...
    virtual int CreateChannel(QString strChannelID, uchar ucType, uchar ucReadWrite, uchar* aLocalIP, int iLocalPort, uchar* aRemoteIP,
        int iRemotePort, uchar* aV3TargetIP, uchar ucTargetNum, ICommunicationChannel*& pChannel) = 0;

    /**
     * 删除通道
     * ：对于通道的删除，如果还有其他对象在引用该通道，则可能引发异常，所以在实现时应采用引用计数的方式，每次调用删除通道函数计数减一，当计数为0时，才真正删除通道对象。计数方式不好，如果一个调用者多次查找如何计数？并且动态创建和删除一般都是一个对象使用。
...
public:
     ...
     *          -4：调用DbbApi.dll错误。
     */
    virtual int GetDeviceState(uint uiStatusID, uchar aStatus[], int& iStatusLen, uint& uiUpdateCount) = 0;
     ...
     *          -1：失败。
     */
    virtual int ReadDataFromDriver(uint uiDataID, uchar aData[], uint& uiDataLen, uint& uiReloadCnt) = 0;
...
public:
     ...
     * @return 1：成功；-1：失败。
     */
    virtual int MsgReceiverRegister(ushort msgT, ushort msgC, QString strRecvName, MessageProcessingHandler msgRecvFunc) = 0;
     ...
     * @return 1:成功；-1:失败
     */
    virtual int AddDestination(unsigned short usMsgDest, unsigned char* szRecver, ICommunicationChannel* pMsgRecvChnl) = 0;
...
public:
     ...
     */
    virtual int Query(time_t& tStartTime, time_t& tEndTime, char szUserName[], uint uiSource, char szObjectID[], uchar ucSrcType,
        uchar ucWarningLevel, uchar ucMainType, uchar ucSubType, QList<TLxTsspLog>& logList) = 0;
     ...
     */
    virtual int Delete(time_t& tStartTime, time_t& tEndTime, char szUserName[], uint uiSource, char szObjectID[], uchar ucSrcType,
...
public:
     ...
     */
    virtual int GetOneInfoItem(unsigned int uiInfoID, unsigned char* pInfo, int& iInfoLen) const = 0;
     ...
     * @return 0 -- 无此信息, 1 -- 成功，-1 -- 输入的信息类型错误。
     */
    virtual int GetOneInfoItemPointer(unsigned int uiInfoID, const unsigned char*& pInfo, int& iInfoLen) const = 0;
     ...
     */
    virtual int UpdateOneInfoItem(unsigned int uiInfoID, unsigned char* pInfo, int iInfoLen) = 0;
     ...
     */
    virtual int SubscribeUpdateMessage(unsigned int uiInfoID, UpdateProcessHandler processFunction) = 0;
...
//
typedef int (*PCreateObjects)(const QDomNode xmlConfigInfo, QMap<quint64, void*>& ObjList);
//
...
public:
     ...
     * @return 状态标识号
     */
    virtual quint64 GetID(ushort usStationID, uchar ucSubSysID, ushort usDeviceID, ushort usType, uchar ucSn) = 0;
     ...
     * @return TRUE--成功；FALSE--失败；成功时：pData中为数据；iDataLen:返回实际读到的长度
     */
    virtual bool GetOneItemByIndex(int iIndex, quint64& ui64StatusID, uchar* pData, int& iDataLen, uint& uiTimeTag) = 0;
...
public:
     ...
     */
    void* GetObject(ushort usStationID, uchar ucSubSysID, ushort usDeviceID, ushort usType, uchar ucSn);
    /**
